...
function(input, output,session) {
 ...
 #  observeEvent(input$myconfirmation_submit, {
 #    if (!is.null (input$myconfirmation_submit) ){ 
 ...
 # })
  ...
  observeEvent (input$submit_slider_range_p1, { 
    ...
    if (input$submit_slider_range_p1){
      ...
      updateSliderInput(session, "rec_range", label = "Concentration range to plot (log units)",
                        min = input$plot_start_p1, max = input$plot_end_p1,step = 0.1,
  ...
  observeEvent (input$submit_slider_range_p2, { 
    ...
    if (input$submit_slider_range_p2){
      ...
      updateSliderInput(session, "lig_range", label = "Concentration range to plot (log units)",
                        min = input$plot_start_p2, max = input$plot_end_p2, step = 1,
  ...
  observeEvent (input$submit_slider_range_kd, { 
    ...
    if (input$submit_slider_range_kd){
      ...
      updateSliderInput(session, "kd_range", label = "Concentration range to plot (log units)",
                        min = input$plot_start_kd, max = input$plot_end_kd, step = 1,
  ...
  output$freePartner2 <- renderText({
    ...
    pc_occ_ligand <- 100*(1-c_free_ligand/input$c_total_ligand)
    #paste("Free ",input$receptor_name," is ",signif(c_free_receptor,digits=3)," pM.",sep="")
    paste(format(signif(c_free_ligand,digits=3), decimal.mark=",")," ", input$unit,sep="")
  })
  ...
  output$InComplexConcentration <- renderText({
    ...
    pc_occ_ligand <- 100*(1-c_free_ligand/input$c_total_ligand)
    #paste("Free ",input$receptor_name," is ",signif(c_free_receptor,digits=3)," pM.",sep="")
    paste(format(signif(c_rlcomplex,digits=3),decimal.mark=",")," ", input$unit,sep="")
  })
  ...
  cxbyligrangerecdata <- reactive({ 
    ...
    if(input$p1concunit == "molar units"){
      ...
      fixed_receptor <- input$c_total_receptor_start
    }
    ...
    if(input$p2concunit == "molar units"){
      ...
      fixed_ligand <- input$c_total_ligand_start
    }
    ...
    # 
    #   #convert receptor back to original units
    #     if (input$p1unit == "ug/ml"){
    #       
    #       fixed_receptor <- input$c_total_receptor *(1/10^12) *(1/1000) * input$mol_weight_p1 * 10^6
    #      
    #       # temp = format  (input$c_total_receptor / 1e6 / input$mol_weight_p1 * 10^12 * 1000, scientific = T)
    #     }else if (input$p1unit == "mg/ml"){
    #       
    #       fixed_receptor <- input$c_total_receptor *(1/10^12) *(1/1000) * input$mol_weight_p1 * 10^3
    # 
    #       # temp = format  (input$c_total_receptor / 1e3 / input$mol_weight_p1 * 10^12 * 1000, scientific = T)
    #     }else if (input$p1unit == "ng/ml"){
    #       
    #       fixed_receptor <- input$c_total_receptor *(1/10^12) *(1/1000) * input$mol_weight_p1 * 10^9
    # 
    #       # temp = format  (input$c_total_receptor / 1e9 / input$mol_weight_p1 * 10^12 * 1000, scientific = T)
    # 
    #     }else if (input$p1unit == "pg/ml"){
    # 
    #       # print("hello")
    #       fixed_receptor <- input$c_total_receptor *(1/10^12) *(1/1000) * input$mol_weight_p1 * 10^12
    #       print ("fixed receptor")
    #       print (fixed_receptor)
    # 
    #       # fixed_receptor = input$c_total_receptor / 1e12/ input$mol_weight_p1 * 10^12 * 1000
    #     }
    #   
    #   #convert ligand back to original units
    #   if (input$p2unit == "ug/ml"){
    #     
    #     fixed_ligand <- input$c_total_ligand *(1/10^12) *(1/1000) * input$mol_weight_p2 * 10^6
    # 
    #   }else if (input$p2unit == "mg/ml"){
    #     
    #     fixed_ligand <- input$c_total_ligand *(1/10^12) *(1/1000) * input$mol_weight_p2 * 10^3
    #     
    #     
    #   }else if (input$p2unit == "ng/ml"){
    #     
    #     fixed_ligand <- input$c_total_ligand *(1/10^12) *(1/1000) * input$mol_weight_p2 * 10^9
    #     
    #     
    #     
    #   }else if (input$p2unit == "pg/ml"){
    #     
    #     # print("hello")
    #     fixed_ligand <- input$c_total_ligand *(1/10^12) *(1/1000) * input$mol_weight_p2 * 10^12
    #     print ("fixed ligand")
    #     print (fixed_ligand)
    #     
    #   }
  ...
  inputCxbyligrangerecplot <- reactive({ 
    ...
    q <- plot_ly (data = mylist,
      ...
      layout(title = paste("Concentration", em(input$receptor_name), "- ", em(input$ligand_name), "complex (pM) at a fixed K<sub>D</sub> of", em(input$KD), "pM"),
             font = list(size = 9),
             yaxis = list (title = paste(input$ligand_name,"-",input$receptor_name," complex",sep="")),  
    ...
    if(input$scale == 1){
      ...
      q %>%
        ...
        add_annotations( text=paste("Concentration of <br>", input$ligand_name, " (", input$p2unit_metric, ") ",sep = ""), xref="paper", yref="paper",
                         x=0, xanchor="left",
  ...
  inputRoccbyrecrangeligplot <- reactive({
    ...
    if(input$scale == 1){
      ...
      q %>%
        ...
        add_annotations( text=paste("Concentration of <br>", input$ligand_name, " (", input$p2unit, ") ",sep = ""), xref="paper", yref="paper",
                         x=0, xanchor="left",
                         y=-0.435, yanchor="bottom",    # Same y as legend below
                         legendtitle=TRUE, showarrow=FALSE,
                         font=list(size=12,face="bold")) 
  ...
  inputCxbyligrangekdplot <- reactive ({
    ...
    if(input$scale == 1){
      ...
      q %>%
        ...
        add_annotations( text= paste("KD", " (", "pM", ") ",sep = ""), xref="paper", yref="paper",
  ...
  inputTime_to_equilibrium_plot <- reactive({
    ...
    inits <- c(R=input$c_total_receptor,L=input$c_total_ligand, X=0)
    pars <- c(konperpms=input$kon*1e-12,koff=koffpers)
    seq.times <- seq(from=0, to=2*time_to_hours*3600, by = 2*time_to_hours*36)   # 00 points-worth
    out <- data.frame(ode(y=inits, times=seq.times, func=binding_model, parms=pars, atol=1e-4,rtol=1e-4))
    time.points <- out$time/3600  # convert to hours
    yaxis.points <- 100*out$L/input$c_total_ligand
...
#         )
  ...
  output$downloadCompetitionPage <- downloadHandler(
    # For PDF output, change this to "report.pdf"
    filename = "competition_dashboard.pdf",
    ...
    content = function(file) {
      ...
      shiny::withProgress(
        message = "Downloading PDF File", {
          ...
          # Set up parameters to pass to Rmd document
          # params <- list(my.table1 = inputTable2())
          params <- list( plot1 = inputfirstPlot_A(),
